...
The addon currently sends `name:CLASS:41/20/0`. Two fields are needed and
...
One bump covers both. `GetRaidRosterInfo(i)` returns subgroup; `UnitRace(unit)`
returns race.

```
RSS1  (today)  header "RSS1"   name:CLASS:41/20/0
RSS2  (new)    header "RSS2"   name:CLASS:41/20/0:3:Draenei
```

Parser accepts both. `player.group` and `player.race` are `null` on RSS1 input.
`.toc` Version goes 1.0 → 2.0.

Every existing test must pass unchanged — that is the back-compat proof.

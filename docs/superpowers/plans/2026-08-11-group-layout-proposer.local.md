...
**Depends on:** `2026-08-11-rss2-wire-format.md` (needs `player.race` for the draenei pass; the rest works without it) and `2026-08-12-guardian-druid-spec.md` (Task 1 uses `bucketOf` on Guardian and `anchorScore` calls `isFeralSpec`, both of which that plan introduces).
...
test('bucketOf: specs map to the right role bucket', () => {
    ...
    assert.strictEqual(E.bucketOf(P('f2', 'DRUID', 'Feral')), 'melee');
    assert.strictEqual(E.bucketOf(P('f3', 'DRUID', 'Guardian')), 'tanks');
...
- **A druid left as `Feral` buckets as melee.** Tab totals cannot tell a bear from a cat, so a tank who was never marked Guardian lands in the melee group. That is the intended fallback — the Guardian plan adds the manual and Raid-Helper paths for correcting it, and nothing here tries to guess.

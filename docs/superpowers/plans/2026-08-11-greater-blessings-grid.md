...
**Depends on:** `2026-08-11-catalog-rules-and-warning-split.md`.
...
```js
    ...
    const BLESSING_PLANS = [
        cls => 'Greater Kings',
        cls => (PHYSICAL_CLASSES.indexOf(cls) !== -1 ? 'Greater Might' : 'Greater Wisdom'),
        cls => (cls === 'WARRIOR' || cls === 'DRUID' ? 'Sanctuary' : 'Greater Salvation'),
        cls => (PHYSICAL_CLASSES.indexOf(cls) !== -1 ? 'Greater Light' : 'Greater Salvation'),
    ];
    ...
    function proposeBlessings(roster, overrides) {
        ...
        const rows = paladins.map((pal, i) => {
            ...
            classes.forEach(cls => {
                ...
                cells[cls] = Object.prototype.hasOwnProperty.call(overrides, key) ? overrides[key] : plan(cls);
            });
...
- **Druid, Shaman and Paladin are in `PHYSICAL_CLASSES`**, so they default to Greater Might. For a resto-heavy comp that is wrong and the raid lead must fix three cells by hand. A smarter default would look at the majority spec among that class in the roster — worth doing only if the manual correction proves annoying.

...
**Goal:** Carry each player's raid subgroup and race from the addon to the web tool, so party-scoped assignments (totems, auras) and draenei placement become possible — without breaking any raider who never updates the addon.

**Architecture:** The web parser learns the new format *first* and keeps accepting the old one, so a mixed raid works during rollout. Only then does the addon start emitting it. `player.group` and `player.race` are `null` for `RSS1` input and for any player the addon could not inspect.
